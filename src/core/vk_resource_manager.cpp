...
ResourceManager::ResourceManager(const Device& deviceWrapper, const VkAllocator& allocator,
    ...
    hardwareCapabilities(deviceWrapper.capabilities), objectStorage(objectStorageIn), geometryStore(geometryStoreIn),
    materialStore(materialStoreIn), graphicsIndex(deviceWrapper.graphicsIndex),
    transferIndex(deviceWrapper.transferIndex), msaaSamples(deviceWrapper.msaaSamples),
    vertices(geometryStoreIn.vertices), meshlets(geometryStoreIn.meshlets),
    meshletVertices(geometryStoreIn.meshletVertices), meshletTriangles(geometryStoreIn.meshletTriangles)
{
...
void ResourceManager::createMeshBuffers()
{
    ...
    if (meshlets.empty()) {
        ...
        createBuffer(bufferSize,
                     ...
                     device, queueFamilyIndices, "MeshletBufferMemory");
    ...
    if (meshletTriangles.empty()) {
        ...
        createBuffer(triBufferSize,
                     ...
                     allocator.allocator, device, queueFamilyIndices, "MeshletTriangleBufferMemory");
